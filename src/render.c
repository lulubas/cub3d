...
/*   Updated: 2024/05/23 23:35:22 by lbastien         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	raycast_and_render(t_data *data)
{
	...
	buffer.image = mlx_get_data_addr(img_buffer, &buffer.pixel_bits, \
					&buffer.line_bytes, &buffer.endian);
	while (x < SCREEN_WIDTH)
	{
		init_rays(x, SCREEN_WIDTH, data->scene);
		perform_dda(data->scene, data->map);
		split_slice(data->scene);
		draw_slice(x, &buffer, data);
		x++;
	}
